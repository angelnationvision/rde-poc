...
const Picker = props => {
  ...
  const copyToClipboard = key => {
    ...
    if (key instanceof Set) {
      item = [...key].map(k =>
        k.startsWith('category:') ? state.categories[k.replace('category:', '')] : state.items[k]
      );
    } else {
      item = key.startsWith('category:')
        ? state.categories[key.replace('category:', '')]
        : state.items[key];
    }
  ...
  const onLoadMore = async () => {
    if (
      !state.pageInfo ||
      state.pageInfo.current_page >= state.pageInfo.total_pages ||
    ...
    const [items, pageInfo] = await getItems(
      state.folder,
    ...
    return (
      <Provider theme={defaultTheme} height="100%">
        <Flex direction="column" height="100%">
          <View padding="size-500">{renderErrorState()}</View>
        </Flex>
      </Provider>
    );
  }
      ...
      <Flex direction="column" height="100%">
        ...
        <View padding="size-100">
          <Flex direction="row" gap="size-100">
            ...
            >
              {block => <Item key={block.key}>{block.name}</Item>}
            </RSPicker>
            {currentBlock.selection === 'multiple' && (
              <ActionButton
                isDisabled={state.selectedItems.size === 0}
                aria-label="Copy"
          ...
          {item => {
            if (item.isFolder) {
              return (
                <Item
                  key={item.key}
                  textValue={item.name}
                  hasChildItems={currentBlock.selection !== 'multiple'}
                >
                  <Folder />
                  <Text>{item.name}</Text>
                  {item.childCount > 0 && <Text slot="description">{item.childCount} items</Text>}
                  {currentBlock.selection === 'single' &&
                    (currentBlock.type === 'any' || currentBlock.type === 'folder') && (
            ...
            return (
              <Item key={item.key} textValue={item.name}>
                ...
                {currentBlock.selection === 'single' &&
                  (currentBlock.type === 'any' || currentBlock.type === 'item') && (
                    <ActionButton aria-label="Copy" onPress={() => copyToClipboard(item.key)}>
                      <Copy />
                    </ActionButton>
                  )}
