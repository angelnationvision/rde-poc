...
export default function decorate(block) {
  ...
  defaultStack.classList.add('navigation-default-stack');
  ...
  if (isHorizontalView) {
    ...
    navBox.appendChild(defaultStack);
  }
  ...
  children.slice(1).forEach(div => {
    ...
    if (isButton && !hasPicture) {
      ...
      const [, id, ariaLabel, brand, btnType, IconPlacement, size, inverse, icon, openInNewTab] = [
        ...div.children,
      ];
      id?.remove();
      brand?.remove();
    ...
    if ((!hasPicture && !isButton) || (hasPicture && pictures.length > 1 && !isHorizontalView)) {
      const subChildren = Array.from(div.children);
      subChildren.forEach((subDiv, index) => {
        if (index === 0) {
          subDiv.classList.add('navigation-column-title');
          if (subDiv.classList.contains('navigation-column-title')) {
            const column = subDiv.closest('.navigation-column');
            if (column && !column.classList.contains('title-parent')) {
              column.classList.add('title-parent');
            }
          }
          if (pictures.length > 1) {
            subDiv.closest('.navigation-column').classList.add('nav-col-images');
          }
          if (!isHorizontalView) {
          ...
          if (!subDiv.closest('.navigation-column').querySelector('.navigation-column-child')) {
            subDiv.classList.add('navigation-column-child');
          } else if (subDiv.querySelector('.button-container')) {
            subDiv.classList.add('nav-col-imglink');
            subDiv
              .closest('.navigation-column')
              .querySelector('.navigation-column-child')
              .appendChild(subDiv);
          } else if (subDiv.querySelector('picture')) {
            subDiv
              .closest('.navigation-column')
              .querySelector('.navigation-column-child')
              .appendChild(subDiv);
    ...
    if (div.classList.contains('navigation-column-card')) {
      // Append directly to navBox (outside of verticalStack)
      navBox.appendChild(div);
    } else if (!isButton || hasPicture) {
      // Append regular columns into the stack
      if (verticalStack) {
        verticalStack.appendChild(div);
      } else {
        defaultStack.appendChild(div);
      }
      // columnCount += 1;
    } else {
      // Append button columns to the button wrapper
