...
addtocalendarlistDiv.addEventListener('click', function () {
  ...
  if (!isClickInsideDropdown && !isClickInsideCalendar) {
    document.querySelectorAll('.leftContainer-2.active-calendar').forEach(function (el) {
...
END:VCALENDAR
  ...
  icon_apple?.addEventListener('click', function () {
    downloadICS();
  });
  icon_outlook?.addEventListener('click', function () {
    downloadICS();
  });
