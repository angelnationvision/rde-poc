...
export const assertCartSummaryProduct =
  (productName, productSku, productQty, productPrice, totalPrice, productPosition) =>
  (elem = '.commerce-cart-wrapper') => {
    cy.get(`${elem} .dropin-cart-item__title`).eq(productPosition).should('contain', productName);
    cy.get(`${elem} .dropin-cart-item__sku`).eq(productPosition).should('contain', productSku);
    ...
    cy.get(`${elem} .dropin-cart-item__total`).eq(productPosition).should('contain', totalPrice);
  };
...
export const assertCartSummaryProductsOnCheckout = (
  ...
  cy.get('.dropin-cart-item__title').eq(productPosition).should('contain', productName);
  cy.get('.dropin-cart-item__sku').eq(productPosition).should('contain', productSku);
  cy.get('.dropin-cart-item__price__quantity').eq(productPosition).should('contain', productQty);
  cy.get('.dropin-cart-item__price').eq(productPosition).should('contain', productPrice);
  cy.get('.dropin-cart-item__total').eq(productPosition).should('contain', totalPrice);
};
...
export const assertSelectedPaymentMethod = (selected_payment_method, index_number) => {
...
export const assertOrderConfirmationCommonDetails = (customerDetails, paymentMethod) => {
  cy.get('.order-confirmation')
    .should('contain', `${customerDetails.firstName}, thank you for your order!`)
...
export const assertOrderConfirmationShippingMethod = customerDeliveryMethod => {
...
export const assertGiftOptionsSummary = (title, price) => {
  ...
  if (price) {
    cy.get(summaryClassName).contains(price).should('exist').should('be.visible');
  }
};
...
export const assertGiftOptionsEmptyForm = className => {
  ...
  cy.get(`${className} ${fields.giftOptionSenderName}`).should('have.class', 'dropin-input--error');
  cy.get(`${className} ${fields.giftOptionMessage}`).should('have.class', 'dropin-textarea--error');
};

export const assertGiftOptionsReadOnlyFormView = () => {
  const summaryClassName = '.cart-gift-options-view--readonly';
  const cartClassName = '.cart-cart-summary-list.cart-cart-summary-list__background--secondary';

  cy.get(summaryClassName)
    .contains('Selected gift order options')
    .should('exist')
    .should('be.visible');
  cy.get(summaryClassName).contains('Use gift receipt').should('exist').should('be.visible');
  cy.get(summaryClassName)
    .contains('Selected gift order options')
    .should('exist')
    .should('be.visible');
  cy.get(summaryClassName)
    ...
    .should('exist')
    .should('be.visible');
  cy.get(summaryClassName).contains('Use printed card').should('exist').should('be.visible');
  cy.get(summaryClassName)
    .contains('Gift wrap this order (+$30.00)')
    .should('exist')
    .should('be.visible');
  cy.get(summaryClassName).contains('Foil Finish Paper').should('exist').should('be.visible');
  cy.get(summaryClassName).contains('giftOptionRecipientName').should('exist').should('be.visible');
  cy.get(summaryClassName).contains('giftOptionSenderName').should('exist').should('be.visible');
  cy.get(summaryClassName).contains('giftOptionMessage').should('exist').should('be.visible');

  cy.get(cartClassName).contains('This item is a gift').click();
  cy.get(cartClassName).contains('Foil Finish Paper').should('exist').should('be.visible');
  cy.get(cartClassName).contains('giftOptionRecipientName').should('exist').should('be.visible');
  cy.get(cartClassName).contains('giftOptionSenderName').should('exist').should('be.visible');
  cy.get(cartClassName).contains('giftOptionMessage').should('exist').should('be.visible');
};
...
export const assertWishlistItem =
  ...
  (elem = '.commerce-wishlist-wrapper') => {
    cy.get(elem).within(() => {
      cy.get('.wishlist-product-item-name').contains(productName).should('be.visible');
      cy.get('.wishlist-product-item-price').contains(productPrice).should('be.visible');
    });
  };
  ...
  (elem = '.commerce-wishlist-wrapper') => {
    cy.get(elem).within(() => {
      cy.get('.wishlist-product-item-options').within(() => {
        cy.get('.wishlist-product-item-option__attribute').contains(attribute).should('be.visible');
        cy.get('.wishlist-product-item-option__label').contains(label).should('be.visible');
      });
    });
  };
