  ...
  try {
    if (typeof document < 'u') {
      const r = document.createElement('style'),
        d = t.styleId;
      for (const e in t.attributes) r.setAttribute(e, t.attributes[e]);
      (r.setAttribute('data-dropin', d), r.appendChild(document.createTextNode(o)));
      const i = document.querySelector('style[data-dropin="sdk"]');
      if (i) i.after(r);
      else {
        const e = document.querySelector('link[rel="stylesheet"], style');
        e ? e.before(r) : document.head.append(r);
      }
    }
  } catch (r) {
...
.product-discovery-results-info{display:none}.product-discovery-results-info__search-phrase--upercase{text-transform:capitalize}@media only screen and (min-width: 768px){.product-discovery-results-info{display:flex;flex-direction:row;gap:var(--spacing-xsmall);width:100%;padding:var(--spacing-xsmall);text-align:center}}`,
  { styleId: 'storefront-product-discovery' }
);
import { jsx as o } from '@dropins/tools/preact-jsx-runtime.js';
import { Render as c } from '@dropins/tools/lib.js';
import { useState as i, useEffect as a } from '@dropins/tools/preact-hooks.js';
...
const h = {
    PLP: {
      Warning: {
        minQueryLength:
          'Your search term {SEARCH_PHRASE} has not reached the minimum of {{minQueryLength}} characters.',
    ...
    Facet: { showMore: 'Show more', showLess: 'Show less', clearAll: 'Clear all' },
    ResultsInfo: { resultsFor: 'Results for', products: 'products' },
    SearchInput: { placeholder: 'Search', viewAllButton: 'View All' },
  },
  S = { Search: h },
  m = { default: S },
  d = ({ children: r }) => {
    ...
    return (
      ...
      o(l, { lang: t, langDefinitions: m, children: r })
    );
  },
  A = new c(o(d, {}));
export { A as render };
