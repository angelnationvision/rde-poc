  ...
  try {
    if (typeof document < 'u') {
      const t = document.createElement('style'),
        n = e.styleId;
      for (const i in e.attributes) t.setAttribute(i, e.attributes[i]);
      (t.setAttribute('data-dropin', n), t.appendChild(document.createTextNode(r)));
      const a = document.querySelector('style[data-dropin="sdk"]');
      if (a) a.after(t);
      else {
        const i = document.querySelector('link[rel="stylesheet"], style');
        i ? i.before(t) : document.head.append(t);
      }
    }
  } catch (t) {
...
const h = {
    ...
    Wishlist: { heading: 'Wishlist {count}', loading: 'Loading...' },
    Alert: {
      addProduct: {
      ...
      moveToCart: { heading: 'Moved to cart', message: '{product} has been moved to your cart' },
      viewWishlist: 'View wishlist',
    },
    Login: { sync: ' to sync your saved items across all your devices.', logIn: 'Log in' },
  },
  u = {
  ...
  f = ({ children: e }) => {
    ...
    return (
      ...
      o(c, { lang: s, langDefinitions: g, children: e })
    );
  },
  P = new n(o(f, {}));
export { P as render };
